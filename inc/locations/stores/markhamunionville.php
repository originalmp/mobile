<!-- START of /inc/locations/stores/markhamunionville.php  -->
 <ul class="store-location">
        <li class="store-image"><img src="http://m.canadacomputers.com/sty/img/location/markham.jpg" alt="Canada Computers and Electronics"></li>

  <li class="store-address"><a href="https://maps.apple.com/?q=4038%20York%20Regional%20Rd%207,%20Markham,%20ON%20L3R%202L5" target="_blank">4038 York Regional 7 East, Markham,Ontario L3R 2L5, Canada</a></li>
  
  
    <li class="store-hours title">Store &amp; Service Centre Hours:</li>
  <li class="store-hours weekdays">Mon-Fri : 10am – 8pm</li>
  <li class="store-hours saturdays">Saturday : 10am – 7pm</li>
  <li class="store-hours sundays">Sunday : 11am – 6pm</li>
  
  
  <a href="tel:905-947-9000"><li class="store-phone"><img class="phone-icon" src="./images/phone.png"> 905-947-9000<img class="chevron-icon" src="./images/chevron.png"></span></li></a>
  
  
  <!-- a class="email" href="mailto:markham@canadacomputers.com">
  <li>E: markham@canadacomputers.com</li>
  </a -->
  <li class="inquiries">For inquiries regarding Service or System build contact our Service Centre:</li>
  <a class="email" href="mailto:markhamservice@canadacomputers.com">
        <li class="store-service-centre"><img class="email-icon" src="./images/email.png"> Contact Service Centre<span class="chevron"><img class="chevron-icon" src="./images/chevron.png"></span></li>
  </a>
  <a href="tel:905-947-8571"><li class="store-hours"><img class="phone-icon" src="./images/phone.png"> 905-947-8571<span class="chevron"><img class="chevron-icon" src="./images/chevron.png"></span></li></a>

</ul>



 <div class="map-container"><embed src="./inc/locations/stores/markhamunionville-map.html"></embed></div>
<div id="store-connections">
  <ul id="bottom-menu" class="row">
    <li class="col-xs-4 col-sm-4 col-md-4"><a class="btn" href="tel:905-947-9000">Call</a></li>
    <li class="col-xs-4 col-sm-4 col-md-4"><a class="btn" href="mailto:markham@canadacomputers.com">Email</a></li>
    <li class="col-xs-4 col-sm-4 col-md-4"><a class="btn" href="https://maps.apple.com/?q=4038%20York%20Regional%20Rd%207,%20Markham,%20ON%20L3R%202L5">Directions</a></li>
  </ul>
</div>
<!-- END /inc/locations/stores/markhamunionville.php  -->